...
class TestBurger(unittest.TestCase):
    ...
    def test_move_ingredient_out_of_range(self):
        ...
        self.burger.ingredients = [self.mock_ingredient1]
        with self.assertRaises(IndexError):
            self.burger.move_ingredient(0, 5)
    ...
    def test_move_ingredient_parametrized(self):
        ...
        test_cases = [
            (0, 2, [2, 1, 3]),  # Перемещение первого на позицию 2
            (2, 0, [3, 1, 2]),  # Перемещение третьего на позицию 0
            (1, 0, [2, 1, 3]),  # Перемещение второго в начало
            (1, 2, [1, 3, 2])  # Перемещение второго в конец
        ]
